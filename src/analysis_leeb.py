...
# NOTE:
#   - t0 is fixed to 0.0
#   - pi is free to allow incomplete down-regulation
...
def _norm_vec(v: np.ndarray) -> np.ndarray:
    """Normalize a vector to [0,1]. If values look like percent, divide by 100 first."""
...
def extract_se(param_name: str, cov, fitted_keys):
    """Extract the SE of a fitted parameter from the covariance matrix."""
...
# Fit the down-regulation signal as fraction transitioned
...
for g, sub in df.groupby("gene"):
    ...
    # SEs from the covariance estimate
...
# Order by t50 for reporting and plotting
...
def pairwise_tests(df_sum, value_col, se_col, out_csv, label):
    """Run pairwise Wald tests for one fitted metric."""
    rows = []
...
def _quick_report(df_comp, title):
    """Print a compact summary of pairwise comparison results."""
    if df_comp.empty:
        print(f"\nNo pairwise results for {title}.")
        return

    shown = df_comp.sort_values("p_adj_holm")[
        ["gene_a", "gene_b", "diff", "se_diff", "p_adj_holm", "q_BH", "significant"]
    ]
...
# Colors ordered by t50 rank (earliest to latest)
t50_palette = [
    "#1976d2",  # deep blue
...
# Fitted curves: predicted expression = 1 - fitted fraction transitioned
...
# ============================================================
# siPum1 vs siNeg analysis (S3C dataset)
# ============================================================
...
# Numeric conversion and NA removal
...
# Normalize per gene within each condition
df_s3c["expr_norm"] = (
...
def compare_two_conditions(fit_table, metric, se_col, out_name):
    """Compare siPum1 and siNeg for one metric within each gene."""
    rows = []
...
# Multiplot: 2 x 3 panels, ordered by original t50 ranking
